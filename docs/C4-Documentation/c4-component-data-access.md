...
- [c4-code-stoat-ferret-db-new.md](./c4-code-stoat-ferret-db-new.md) -- Additional repository types: TimelineRepository, BatchRepository, ProxyRepository; preview and waveform models
- [c4-code-alembic.md](./c4-code-alembic.md) -- Alembic migration environment: run_migrations_offline(), run_migrations_online(), env.py
- [c4-code-alembic-versions.md](./c4-code-alembic-versions.md) -- 7 schema migrations: initial_schema → audit_log → projects → clips → transitions_effects → preview_sessions → thumbnail_strips
