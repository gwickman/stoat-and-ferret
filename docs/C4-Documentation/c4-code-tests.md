...
- **Name**: Tests Root
- **Description**: Root-level test files covering core infrastructure -- repositories, models, FFmpeg integration, WebSocket, logging, PyO3 bindings, audio builders, transition builders, event loop responsiveness, and observability
- **Location**: `tests/`
...
### Test Inventory (~606 tests across 26 test files + 2 helper files)

| File | Tests | Coverage |
|------|-------|----------|
| conftest.py | 0 (fixtures) | Shared session/function fixtures, FFmpeg/FFprobe detection, sample video generation |
...
| test_database_startup.py | 4 | Database initialization during application lifespan |
...
| test_audit_logging.py | 17 | Audit log writing and query interface |
...
| test_ffmpeg_observability.py | 6 | ObservableFFmpegExecutor metrics and logging |
| test_integration.py | 13 | Cross-component integration scenarios |
| test_logging.py | 4 | Structured logging setup and formatters |
| test_logging_startup.py | 20 | Structured logging configuration during application startup |
...
| test_orphaned_settings.py | 6 | Settings wiring (debug, ws_heartbeat_interval) to consumers |
| test_event_loop_responsiveness.py | 1 | Event loop latency under load |

### Root Test Files

#### conftest.py (Shared Fixtures)
- `_ffmpeg_available() -> bool` -- Checks if ffmpeg binary is in PATH
- `_ffprobe_available() -> bool` -- Checks if ffprobe binary is in PATH
- `requires_ffmpeg` marker -- Skips tests when ffmpeg not available
- `requires_ffprobe` marker -- Skips tests when ffprobe not available
- `sample_video_path(tmp_path_factory) -> Path` -- Session-scoped fixture generating 1s 320x240 H.264+AAC test video
- `video_only_path(tmp_path_factory) -> Path` -- Session-scoped fixture generating 1s video-only (no audio) file
- `project_factory() -> ProjectFactory` -- Function-scoped ProjectFactory builder

#### factories.py (Test Data Builders)
- `make_test_video(**kwargs: object) -> Video` (line 21) -- Creates Video with sensible defaults, accepts kwargs overrides
- `ProjectFactory` -- Builder class for creating projects; fluent API: `.with_name()`, `.with_output()`, `.with_clip()`, `.build()`, `.create_via_api()`
- `_ClipConfig` -- Internal helper for ProjectFactory clip configuration
...
#### test_logging_startup.py (20 tests)
- Validates configure_logging with various parameters
- Tests: idempotency, file handler creation, backup count, max bytes, log level propagation
...
- `stoat_ferret.db.audit` (AuditLogger)
...
- `stoat_ferret.api.settings` (Settings)
...
classDiagram
    namespace TestLayers {
        ...
        class ModelTests {
            +test_clip_model: 11
            +test_db_schema: 9
            +test_database_startup: 4
            +test_audit_logging: 17
        }
        class FFmpegTests {
            +test_executor: 28
            +test_ffprobe: 14
            +test_ffmpeg_observability: 6
        }
        class BuilderTests {
        ...
        class InfraTests {
            +test_logging: 4
            +test_logging_startup: 20
            +test_observable: 18
            +test_orphaned_settings: 6
        }
        class WSTests {
            +test_websocket: 13
            +test_ws_endpoint: 10
...
- **Test Structure**: Tests organized by concern (repositories, models, FFmpeg, builders, logging, API, WebSocket)
- **Observability Tests**: test_ffmpeg_observability.py validates Prometheus metrics and structured log output
- **Startup Tests**: test_database_startup.py and test_logging_startup.py validate application initialization paths
