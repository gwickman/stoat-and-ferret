...
- **Total Tests**: 3 verified
- **Test Files**: 1

| File | Test Count | Description |
|------|-----------|-------------|
| test_import_fallback.py | 3 | stoat_ferret_core fallback stubs on ImportError |
...
- `test_fallback_stub_classes_raise_runtime_error(self) -> None` (line 61)
  - Tests 12 stub class definitions raise RuntimeError when instantiated
  - Classes tested: Clip, ClipValidationError, FrameRate, Position, Duration, TimeRange, DrawtextBuilder, SpeedControl, FFmpegCommand, Filter, FilterChain, FilterGraph
...
- `stoat_ferret_core` — The fallback import path being tested
  - Tested class exports: Clip, ClipValidationError, FrameRate, Position, Duration, TimeRange, DrawtextBuilder, SpeedControl, FFmpegCommand, Filter, FilterChain, FilterGraph
...
classDiagram
    ...
    class StoatFerretCore {
        ...
        DrawtextBuilder() RuntimeError
        SpeedControl() RuntimeError
