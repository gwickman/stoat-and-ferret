...
- Effect type discovery: `GET /api/v1/effects` returns all 41 supported effects with parameter schemas. DSP categories include voice processing (`noise_reduction`, `deesser`, `deplosive`, `time_stretch`) and mastering (`mastering_limiter`, `loudness_normalize`, `parametric_eq`, `multiband_compressor`).
