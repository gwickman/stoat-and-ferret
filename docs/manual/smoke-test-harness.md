...
| v095 | `tests/smoke/test_subtitle_injection.py` — three HTTP-layer smoke tests verifying subtitle injection rejection via `source_path` and `font_color` (BL-595/BL-596) and valid subtitle acceptance. FFmpeg-independent — no `STOAT_TEST_FFMPEG` required. Reference this file for regression verification of subtitle filter-option injection fixes. |
