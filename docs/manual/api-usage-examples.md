...
- **Long-poll pattern (BL-277).** Scan and generic background jobs expose `GET /api/v1/jobs/{job_id}/wait?timeout=N` which blocks up to `N` seconds (1 ≤ N ≤ 3600) and returns HTTP 408 `JOB_WAIT_TIMEOUT` when the deadline passes without reaching a terminal state. Terminal states are `complete`, `failed`, `timeout`, `cancelled`; see `src/stoat_ferret/api/routers/jobs.py`. Callers may resume waiting by calling `/wait` again. **Render jobs use a separate namespace** — poll `GET /api/v1/render/{job_id}` until `status ∈ {completed, failed, cancelled}` (lowercase; distinct from `/api/v1/jobs` terminal state names).
- **WebSocket notifications.** Every state change documented below also publishes a `/ws` event (see `docs/design/01-release-01/Phase 06 Design/` and the WebSocket replay pattern in `FRAMEWORK_CONTEXT.md` — subscribers can replay missed events by supplying the last `event_id` via the `Last-Event-ID` header on reconnect).
...
(see `docs/design/01-release-01/02-architecture.md`).
