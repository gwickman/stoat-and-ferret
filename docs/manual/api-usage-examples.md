...
- **Long-poll pattern (BL-277).** Scan and generic background jobs expose `GET /api/v1/jobs/{job_id}/wait?timeout=N` which blocks up to `N` seconds (1 ≤ N ≤ 3600) and returns HTTP 408 `JOB_WAIT_TIMEOUT` when the deadline passes without reaching a terminal state. Terminal states are `completed`, `failed`, `timeout`, `cancelled`; see `src/stoat_ferret/api/routers/jobs.py`. Callers may resume waiting by calling `/wait` again. Render jobs also use `completed` as terminal success — both `/api/v1/jobs/{job_id}` and `/api/v1/render/{job_id}` return `status: "completed"` on terminal success.
