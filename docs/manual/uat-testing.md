...
| 713 | `tests/uat/journeys/j_audio_effect.py` | **Audio Effect Dispatch** — Creates a project with a volume=2.0 audio effect clip via the HTTP API, renders baseline and with-effect versions, and (when STOAT_TEST_FFMPEG=1) asserts audio RMS changed by >= 5 dB via the render-output oracle. Independent. |
| 714 | `tests/uat/journeys/j_crop.py` | **Crop Effect** — Creates a project with a crop effect via the HTTP API, submits a render, and (when STOAT_TEST_FFMPEG=1) asserts output dimensions match the crop parameters via ffprobe and the render-output oracle. Independent. |
| 715 | `tests/uat/journeys/j_preview_parity.py` | **Multi-Clip Preview Composition** — Creates a two-clip project via the HTTP API, starts a preview session, and verifies the session is accepted (HTTP 202). When STOAT_TEST_FFMPEG=1, polls until the session reaches ready state, confirming the multi-clip composition graph is passed to FFmpeg without falling back to first-clip-only. Covers BL-797-AC-4. Independent. |
| 716 | `tests/uat/journeys/j_preview_seek.py` | **Preview Seek Position** — Creates a single-clip project via the HTTP API, starts a preview session, seeks to position 5.0, and verifies the seek returns HTTP 200. Requires STOAT_TEST_FFMPEG=1 for the live preview and seek steps. Covers BL-798-AC-4. Independent. |
...
713 (audio-effect-dispatch) ← independent (STOAT_TEST_FFMPEG=1 required for oracle assertions)
714 (crop-effect)            ← independent (STOAT_TEST_FFMPEG=1 required for oracle assertions)
715 (preview-parity)         ← independent (STOAT_TEST_FFMPEG=1 required for session polling)
716 (preview-seek)           ← independent (STOAT_TEST_FFMPEG=1 required for live seek)
717 (version-restore-roundtrip) ← independent (requires live server with GUI)
```

If journey 201 fails, journeys 202, 203, and 402 are skipped automatically. If journey 205 fails, journey 401 is skipped. Journeys 204, 403, 404, and 501 always run regardless of other journey results. If journey 501 fails, journeys 502, 503, and 504 are skipped automatically. Journeys 604 and 605 always run regardless of other journey results. If journey 701 fails, journeys 702 and 703 are skipped. Journeys 704, 705, 706, 711, 712, 713, 714, 715, 716, and 717 always run regardless of other journey results. Journey 711 (in-point-trim) requires STOAT_TEST_FFMPEG=1 for full oracle verification; without it, the browser navigation step still executes. Journey 712 (multi-clip-audio) requires STOAT_TEST_FFMPEG=1 for oracle assertions; without it, the browser navigation step still executes. Journey 713 (audio-effect-dispatch) requires STOAT_TEST_FFMPEG=1 for oracle assertions; without it, the browser navigation step still executes. Journey 714 (crop-effect) requires STOAT_TEST_FFMPEG=1 for oracle assertions; without it, the browser navigation step still executes. Journey 715 (preview-parity) requires STOAT_TEST_FFMPEG=1 for session polling; without it, the journey starts the preview and returns a scaffold result. Journey 716 (preview-seek) requires STOAT_TEST_FFMPEG=1 for the live preview and seek steps; without it, the journey skips after project creation. Journey 717 (version-restore-roundtrip) requires a live server with the GUI Restore button (Feature 004 must be merged); it drives restore through the browser, not via API.
