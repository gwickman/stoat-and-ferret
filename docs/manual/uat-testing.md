...
| 716 | `tests/uat/journeys/j_unknown_effect_failclosed.py` | **Unknown Effect Fail-Closed** — Creates a project with an unknown effect type via the HTTP API, submits a render, polls to terminal, and asserts the render job fails rather than silently succeeding (BL-795). Independent. |
