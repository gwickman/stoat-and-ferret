...
| 720 | `tests/uat/journeys/j_version_restore.py` | **Version Restore Round-Trip** — Navigates to a project with a generator clip, saves a version via GUI, deletes the generator clip to mutate live state, restores the saved version, and asserts the generator clip re-appears with preserved generator_params metadata. Covers BL-799-AC-6 and BL-842. Requires live server with GUI and a pre-seeded generator clip. Independent. |
