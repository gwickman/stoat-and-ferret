...
| 713 | `tests/uat/journeys/j_transitions.py` | **Transitions Wiring** — Creates an independent two-clip project, saves a wipeleft/0.35 transition via the timeline transitions API, submits a multi-clip render, and (when STOAT_TEST_FFMPEG=1) asserts both source-frame order and transition style at the seam via the render-output oracle. Independent. |
| 714 | `tests/uat/journeys/j_nondefault_fps.py` | **Non-Default FPS** — Creates a project with output_fps=24 via the HTTP API, generates a 24fps fixture, submits a render, and (when STOAT_TEST_FFMPEG=1) asserts r_frame_rate=24/1 via the render-output oracle. Independent. |
| 715 | `tests/uat/journeys/j_audio_effect.py` | **Audio Effect Dispatch** — Creates a project with a volume=2.0 audio effect clip via the HTTP API, renders baseline and with-effect versions, and (when STOAT_TEST_FFMPEG=1) asserts audio RMS changed by >= 5 dB via the render-output oracle. Independent. |
| 716 | `tests/uat/journeys/j_unknown_effect_failclosed.py` | **Unknown Effect Fail-Closed** — Attempts to apply an unknown effect type via POST /projects/{id}/clips/{id}/effects and asserts 400 EFFECT_NOT_FOUND at creation time. No render is submitted — the server rejects the unknown effect at the creation boundary (fail-closed contract, BL-795). Independent. |
| 717 | `tests/uat/journeys/j_crop.py` | **Crop Effect** — Creates a project with a crop effect via the HTTP API, submits a render, and (when STOAT_TEST_FFMPEG=1) asserts output dimensions match the crop parameters via ffprobe and the render-output oracle. Independent. |
| 718 | `tests/uat/journeys/j_preview_parity.py` | **Multi-Clip Preview Composition** — Creates a two-clip project via the HTTP API, starts a preview session, and verifies the session is accepted (HTTP 202). When STOAT_TEST_FFMPEG=1, polls until the session reaches ready state, confirming the multi-clip composition graph is passed to FFmpeg without falling back to first-clip-only. Covers BL-797-AC-4. Independent. |
| 719 | `tests/uat/journeys/j_preview_seek.py` | **Preview Seek Position** — Creates a single-clip project via the HTTP API, starts a preview session, seeks to position 5.0, and verifies the seek returns HTTP 200. Requires STOAT_TEST_FFMPEG=1 for the live preview and seek steps. Covers BL-798-AC-4. Independent. |
| 720 | `scripts/uat_journey_720.py` | **Version Restore Round-Trip** — Navigates to a project, records clip count, saves a version via GUI, adds a clip to modify the live timeline, navigates to the Versions section in ProjectDetails, clicks the Restore button on the saved version, and asserts the live timeline reverted to the saved snapshot (clip count matches). Covers BL-799-AC-6. Requires live server with GUI. Independent. |
| 721 | `tests/uat/journeys/j_split_preserve.py` | **Split Preserve** — Creates a project with a generator clip and effect, splits with copy_full_stack policy, and asserts both children carry parent effects and migration_report entries. Independent. |
...
713 (transitions-wiring)         ← independent (STOAT_TEST_FFMPEG=1 required for oracle assertions)
714 (nondefault-fps)             ← independent (STOAT_TEST_FFMPEG=1 required for oracle assertions)
715 (audio-effect-dispatch)      ← independent (STOAT_TEST_FFMPEG=1 required for oracle assertions)
716 (unknown-effect-failclosed)  ← independent (no FFmpeg required for fail-closed assertion)
717 (crop)                       ← independent (STOAT_TEST_FFMPEG=1 required for oracle assertions)
718 (preview-parity)             ← independent (STOAT_TEST_FFMPEG=1 required for session polling)
719 (preview-seek)               ← independent (STOAT_TEST_FFMPEG=1 required for live seek)
720 (version-restore-roundtrip)  ← independent (requires live server with GUI)
721 (split-preserve)             ← independent
```

If journey 201 fails, journeys 202, 203, and 402 are skipped automatically. If journey 205 fails, journey 401 is skipped. Journeys 204, 403, 404, and 501 always run regardless of other journey results. If journey 501 fails, journeys 502, 503, and 504 are skipped automatically. Journeys 604 and 605 always run regardless of other journey results. If journey 701 fails, journeys 702 and 703 are skipped. Journeys 704, 705, 706, 711–721 always run regardless of other journey results. Journey 711 (in-point-trim) requires STOAT_TEST_FFMPEG=1 for full oracle verification; without it, the browser navigation step still executes. Journey 712 (multi-clip-audio) requires STOAT_TEST_FFMPEG=1 for oracle assertions; without it, the browser navigation step still executes. Journey 713 (transitions-wiring) requires STOAT_TEST_FFMPEG=1 for oracle assertions; without it, the browser navigation step still executes. Journey 714 (nondefault-fps) requires STOAT_TEST_FFMPEG=1 for oracle assertions; without it, the browser navigation step still executes. Journey 715 (audio-effect-dispatch) requires STOAT_TEST_FFMPEG=1 for oracle assertions; without it, the browser navigation step still executes. Journey 716 (unknown-effect-failclosed) does not require STOAT_TEST_FFMPEG; it posts an unknown effect via `POST /projects/{id}/clips/{id}/effects` and asserts HTTP 400 `EFFECT_NOT_FOUND` at creation time; no render is submitted. Journey 717 (crop) requires STOAT_TEST_FFMPEG=1 for oracle assertions; without it, the browser navigation step still executes. Journey 718 (preview-parity) requires STOAT_TEST_FFMPEG=1 for session polling; without it, the journey starts the preview and returns a scaffold result. Journey 719 (preview-seek) requires STOAT_TEST_FFMPEG=1 for the live preview and seek steps; without it, the journey skips after project creation. Journey 720 (version-restore-roundtrip) requires a live server with the GUI Restore button (Feature 004 must be merged); it drives restore through the browser, not via API. Journey 721 (split-preserve) runs entirely via the HTTP API with no FFmpeg requirement.
