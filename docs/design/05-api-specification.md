...
Returns all registered effects from the `EffectRegistry` with parameter schemas, AI hints, and a sample filter preview generated by the Rust builders.
...
#### Future: Preview Effect Filter
```http
POST /effects/preview
```

*Not yet implemented.* Will preview what filter string would be generated without applying to a clip.

---

#### Future: Update/Delete Effect
```http
PATCH /projects/{project_id}/clips/{clip_id}/effects/{effect_id}
DELETE /projects/{project_id}/clips/{clip_id}/effects/{effect_id}
```

*Not yet implemented.* Currently effects are append-only via the apply endpoint.
