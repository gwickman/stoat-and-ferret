...
| `tests/test_contract/test_subtitle.py` | Subtitle builder contract tests (BL-518/519/520/583/584/585/586, v092–v094): SubtitleScriptBuilder timed drawtext chains (BL-518), BurnedSubtitleBuilder SRT/ASS sidecar burn-in (BL-519), soft subtitle MP4/MKV embedding (BL-520), soft-subtitle `-map` index fix (BL-583), Windows font path escaping for SubtitleScriptBuilder (BL-585), force_style single-quote safety (BL-586), and render-command argv limit guard (BL-584). Builders are implemented in Rust at `rust/stoat_ferret_core/src/ffmpeg/subtitles.rs`. Most tests run without FFmpeg; two ffprobe assertion tests are gated by `STOAT_TEST_FFMPEG=1`. |
