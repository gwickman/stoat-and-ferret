...
def upgrade() -> None:
    ...
    existing = {
        row[1]
        for row in bind.execute(sa.text("PRAGMA table_info(videos)")).fetchall()
    }
    if "subtitle_count" not in existing:
        op.execute(
            "ALTER TABLE videos ADD COLUMN subtitle_count INTEGER NOT NULL DEFAULT 0"
        )
    if "data_count" not in existing:
        op.execute(
            "ALTER TABLE videos ADD COLUMN data_count INTEGER NOT NULL DEFAULT 0"
        )
    if "subtitle_streams" not in existing:
        op.execute(
            "ALTER TABLE videos ADD COLUMN subtitle_streams TEXT NOT NULL DEFAULT '[]'"
        )
