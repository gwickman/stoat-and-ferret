...
async def test_hls_hardcut_two_clip_no_transition_preview(
    smoke_client: httpx.AsyncClient,
    videos_dir: Path,
) -> None:
    """FR-002-AC-1: two-clip no-transition project generates HLS playlist without error.

    Creates a two-clip project with no stored transition (hard-cut seam, BL-887/F005),
    requests a preview session, and asserts the session starts (202), reaches ready
    status, and carries a non-null manifest_url.
    """
    timeline_data = await create_adjacent_clips_timeline(smoke_client, videos_dir)
    project_id = timeline_data["project_id"]
    ...
    assert final_status == "ready", (
        f"Expected status 'ready' for two-clip hard-cut preview, got: {final_status!r}"
    )
    assert body.get("manifest_url") is not None, (
        "Expected non-null manifest_url when status is 'ready'"
    )
