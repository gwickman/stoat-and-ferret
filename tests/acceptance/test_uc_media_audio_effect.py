...
"""Acceptance test: uc_media_audio_effect — single-clip render with audio effect (BL-794 AC-3).

Generates an audio+video fixture via the amerge stereo pattern, builds the FFmpeg command
via build_command_for_job with a VOLUME effect, executes it, then asserts the oracle:
audio RMS changed by >= 5 dB and video SSIM >= 0.99 (byte-stable).

All tests are FFmpeg-gated (STOAT_TEST_FFMPEG=1).
Run with: STOAT_TEST_FFMPEG=1 uv run pytest tests/acceptance/test_uc_media_audio_effect.py -v
"""
...
_PROJECT_ID = "proj-audio-effect-001"
...
@_FFMPEG_SKIP
@pytest.mark.asyncio
async def test_audio_effect_dispatch_single_clip(tmp_path: Path) -> None:
    """Single-clip render: volume=2.0 effect raises RMS by >= 5 dB; video SSIM >= 0.99.

    BL-794 AC-3: acceptance test verifying that audio effects are dispatched to the
    audio chain (not silently dropped into the video filtergraph).
    """
    fixture = _make_audio_video_fixture(tmp_path / "fixture.mp4")

    vid_id = "vid-audio-eff"
    ...
    # --- Render WITH volume=2.0 effect: measure effect RMS ---
    ...
    # Audio RMS must differ by >= 5 dB (volume=2.0 ≈ +6 dB)
