...
```bash
git checkout -b v009/01-observability-pipeline/001-ffmpeg-observability
```
...
**Feature:** 001-ffmpeg-observability

## Input Documents

Use the Read tool to read these files before starting:

1. **Requirements:** `comms/inbox/versions/execution/v009/01-observability-pipeline/001-ffmpeg-observability/requirements.md`
2. **Implementation Plan:** `comms/inbox/versions/execution/v009/01-observability-pipeline/001-ffmpeg-observability/implementation-plan.md`
...
Use Write tool to create these in `comms/outbox/versions/execution/v009/01-observability-pipeline/001-ffmpeg-observability/`:
...
You have file-level tools only (Read, Write, Edit, Bash). MCP tools are not available. Do not edit state files (backlog.json, learnings.json, etc.) directly.