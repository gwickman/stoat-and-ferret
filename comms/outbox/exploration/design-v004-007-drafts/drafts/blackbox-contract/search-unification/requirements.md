...
`AsyncInMemoryVideoRepository` uses substring match (`query_lower in v.filename.lower()` at `async_repository.py:301-308`) while `AsyncSQLiteVideoRepository` uses FTS5 prefix match (`'"{query}"*'` at `async_repository.py:186-195`). Three known gaps identified in Task 006 (R3 resolved): (1) substring vs prefix, (2) multi-word phrase handling, (3) field scope. Per-token `startswith` closes gap #1; gaps #2–#3 are minor for current usage patterns.
